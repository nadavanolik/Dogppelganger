...
function Game() {
  ...
  const pool = useMemo(() => state.matches.filter((m) => m.shared && m.status === "done"), [state.matches]);
    ...
    return (
      <AppShell>
        <div className="card-pop max-w-xl mx-auto p-8 text-center">
          ...
          <p className="text-muted-foreground">Once people share to the gallery, the game fills up automatically.</p>
        </div>
      </AppShell>
    );
  }
        ...
        <div className="card-pop p-8">
          ...
          <div className="grid grid-cols-2 gap-3 mt-6">
            {choices.map((c) => {
              ...
              const state =
                picked == null ? "" : c === picked ? (isRight ? "bg-mint" : "bg-destructive text-destructive-foreground") : isRight ? "bg-mint" : "opacity-60";
              return (
                <button
                  key={c}
          ...
          {picked && (
            <div className="mt-6 text-center">
              <div className="text-2xl">{correct ? "🎉 Correct!" : "❌ Nope."} The answer was {target.breedEmoji} {target.breedName}.</div>
              <button onClick={() => { setPicked(null); setI(i + 1); }} className="btn-pop btn-pop-hover bg-primary text-primary-foreground mt-3 px-5 py-2">Next →</button>
            </div>
          )}
        </div>
      </div>
