...
function Inner() {
  ...
  const match = lobby?.currentMatchId
    ? state.matches.find((m) => m.id === lobby.currentMatchId)
    : null;

  const choices = useMemo(() => {
    if (!match) return [];
    const others = BREEDS.filter((b) => b.name !== match.breedName)
  ...
  if (!lobby)
    return (
      <div className="card-pop p-8 text-center">
        Lobby closed.{" "}
        <Link to="/lobbies" className="underline">
          Back
        </Link>
      </div>
    );
        ...
        {!match ? (
          <div className="mt-8 text-center">
            ...
            <p className="text-muted-foreground">
              Host, kick off the first round when players are in.
            </p>
            {lobby.hostId === me.id && (
              <button
                onClick={() => {
                  setPicked(null);
                  advanceLobbyRound(lobby.id);
                }}
                className="btn-pop btn-pop-hover bg-primary text-primary-foreground px-5 py-2 mt-4"
              >
                Start round
              </button>
            )}
          ...
          <div className="mt-6">
            <div className="text-sm font-bold text-muted-foreground text-center">
              Everyone sees the same photo. Pick the breed.
            </div>
            <div className="flex justify-center mt-3">
              {match.humanImg.startsWith("data:") ? (
                <img
                  src={match.humanImg}
                  className="h-56 w-56 rounded-2xl border-2 border-[var(--ink)] object-cover"
                  alt=""
                />
              ) : (
              ...
              {choices.map((c) => {
                ...
                return (
                  <button
                    ...
                    onClick={() => {
                      setPicked(c);
                      submitLobbyGuess(lobby.id, right);
                    }}
            ...
            {picked && (
              <div className="text-center mt-6">
                <div className="text-2xl">
                  Answer: {match.breedEmoji} {match.breedName}
                </div>
                {lobby.hostId === me.id && (
                  <button
                    onClick={() => {
                    ...
                    className="btn-pop btn-pop-hover bg-primary text-primary-foreground px-5 py-2 mt-3"
                  >
                    Next round →
                  </button>
                )}
      ...
      <aside className="card-pop p-5 h-fit">
        ...
        <ul className="mt-3 space-y-2">
          {[...lobby.players]
            ...
            .map((p, i) => (
              <li
                key={p.id}
                className={`flex items-center justify-between px-3 py-2 rounded-xl border-2 border-[var(--ink)] ${p.id === me.id ? "bg-sunshine" : "bg-card"}`}
              >
        ...
        {inGame ? (
          <button
            ...
            className="btn-pop btn-pop-hover bg-card w-full py-2 mt-4"
          >
            Leave lobby
          </button>
        ) : (
          <Link
            to="/lobbies"
            className="btn-pop btn-pop-hover bg-primary text-primary-foreground w-full py-2 mt-4 block text-center"
          >
            Back to lobbies
          </Link>
        )}
        <div className="mt-4 text-xs text-muted-foreground">
          Prototype: shared state is per-browser. In production this room would sync over
