...
function Result() {
  ...
  if (!m) {
    return (
      <AppShell>
        <div className="card-pop p-8 text-center">Match not found.</div>
      </AppShell>
    );
  }
        ...
        {!done ? (
          <div className="text-center py-8">
            ...
            <div className="mt-4 font-display text-3xl font-black">
              {m.status === "queued" ? "In the queue…" : "Dogifying…"}
            </div>
            <p className="text-muted-foreground mt-1">
              {m.urgent
                ? "🚨 Urgent priority — coming right up."
          ...
          <>
            ...
            <div className="mt-8 flex items-center justify-center gap-6">
              ...
              <div
                className={`h-36 w-36 rounded-2xl border-2 border-[var(--ink)] bg-gradient-to-br ${m.breedBg} flex items-center justify-center text-8xl relative overflow-hidden`}
              >
                {m.breedImage ? (
                  <img
                    src={m.breedImage}
            ...
            <div className="mt-8 flex flex-wrap gap-3 justify-center">
              ...
              <Link to="/upload" className="btn-pop btn-pop-hover bg-sunshine px-5 py-2">
                🔁 Try another
              </Link>
            </div>
          </>
