...
function Index() {
  ...
  return (
    <AppShell>
      {state.user ? <SignedInHome /> : <PublicLanding />}
    </AppShell>
  );
}
  ...
  return (
    <>
      <section className="grid md:grid-cols-2 gap-8 items-center">
        <div>
          <div className="inline-block px-3 py-1 rounded-full bg-mint border-2 border-[var(--ink)] text-sm font-bold">🐾 the great human-to-dog census</div>
          <h1 className="mt-4 font-display text-6xl md:text-7xl font-black leading-[0.95]">
            Find out what <span className="bg-primary text-primary-foreground px-2 rounded-2xl border-2 border-[var(--ink)] inline-block rotate-[-1.5deg]">dog</span> you actually are.
          </h1>
          ...
          <div className="mt-6 flex flex-wrap gap-3">
            <Link to="/signup" className="btn-pop btn-pop-hover bg-primary text-primary-foreground px-5 py-3 text-lg">🐕 Sign up free</Link>
            <Link to="/login" className="btn-pop btn-pop-hover bg-sunshine px-5 py-3 text-lg">Log in</Link>
          </div>
          <div className="mt-6 flex items-center gap-2 text-sm text-muted-foreground">
            <span className="inline-flex -space-x-2">
              {["🧑", "👩", "🧔", "👵"].map((e, i) => (
                <span key={i} className="h-8 w-8 rounded-full bg-sunshine border-2 border-[var(--ink)] flex items-center justify-center">{e}</span>
              ))}
            </span>
        ...
        <div className="relative">
          <div className="card-pop p-6 rotate-[2deg]">
            ...
            <div className="flex items-center gap-4">
              <div className="h-32 w-32 rounded-2xl border-2 border-[var(--ink)] bg-sky flex items-center justify-center text-6xl">🧑</div>
              <div className="text-4xl">→</div>
              <div className="h-32 w-32 rounded-2xl border-2 border-[var(--ink)] bg-gradient-to-br from-amber-200 to-orange-300 flex items-center justify-center text-6xl">🐕</div>
            </div>
      ...
      <section className="mt-16">
        ...
        <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-5 relative">
          {shared.map((m) => <DogCard key={m.id} match={m} />)}
        </div>
      </section>

      <section className="mt-16 grid md:grid-cols-3 gap-5">
        {[
          { emoji: "🎮", title: "Play match games", body: "Solo drills or live lobbies with friends." },
          { emoji: "💬", title: "Argue in the forum", body: "Posts, comments, media, likes, dislikes." },
          { emoji: "💌", title: "DM your match", body: "Slide into another dog's inbox. 1:1 with photos." },
        ].map((c, i) => (
      ...
      <section className="mt-16 card-pop p-8 text-center bg-sunshine">
        ...
        <div className="mt-4 flex gap-3 justify-center flex-wrap">
          <Link to="/signup" className="btn-pop btn-pop-hover bg-primary text-primary-foreground px-6 py-3 text-lg">Sign up</Link>
          <Link to="/login" className="btn-pop btn-pop-hover bg-card px-6 py-3 text-lg">I have an account</Link>
        </div>
      </section>
    </>
  );
      ...
      {queue.length > 0 && (
        <section className="card-pop p-5">
          <div className="flex items-center justify-between">
            ...
            <Link to="/profile" className="text-xs underline">manage →</Link>
          </div>
              ...
              <li key={m.id} className="flex items-center gap-3 p-2 rounded-xl bg-muted border-2 border-[var(--ink)]">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="h-1.5 rounded-full bg-card border border-[var(--ink)] mt-1 overflow-hidden">
                    <div className={`h-full ${m.status === "queued" ? "w-1/4 bg-sunshine" : "w-2/3 bg-primary animate-pulse"}`} />
                  </div>
                </div>
              </li>
            ))}
      ...
      <section className="grid lg:grid-cols-2 gap-6">
        <div className="card-pop p-6">
          <div className="flex items-center justify-between mb-3">
            ...
            <div className="flex gap-2">
              <Link to="/forum/new" className="btn-pop btn-pop-hover bg-primary text-primary-foreground px-3 py-1 text-xs">＋ Post</Link>
              <Link to="/forum" className="text-xs underline self-center">all →</Link>
            </div>
          </div>
          <ul className="space-y-2">
            {recentPosts.map((p) => (
              <li key={p.id}>
                <Link to="/forum/$id" params={{ id: p.id }} className="block p-3 rounded-xl border-2 border-[var(--ink)] bg-card hover:bg-muted transition">
            ...
            {recentPosts.length === 0 && <div className="text-sm text-muted-foreground">No posts yet.</div>}
          </ul>
        </div>
        ...
        <div className="card-pop p-6">
          <div className="flex items-center justify-between mb-3">
            ...
            <Link to="/gallery" className="text-xs underline">browse all →</Link>
          </div>
          {galleryStrip.length === 0 ? (
            <div className="text-sm text-muted-foreground">No shared matches yet — share one from your profile.</div>
          ) : (
