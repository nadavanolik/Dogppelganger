    ...
    <AppShell>
      <div className="max-w-3xl mx-auto space-y-6">
        ...
        <article className="card-pop p-6">
          ...
          {post.media && (
            <img src={post.media} className="mt-4 rounded-xl border-2 border-[var(--ink)]" alt="" />
          )}
          <div className="mt-4 flex items-center gap-2">
            <button
              onClick={() => toggleReact({ postId: post.id }, "likes")}
              className="btn-pop btn-pop-hover bg-card px-3 py-1 text-sm"
            >
              👍 {post.likes.length}
            </button>
            <button
              onClick={() => toggleReact({ postId: post.id }, "dislikes")}
              className="btn-pop btn-pop-hover bg-card px-3 py-1 text-sm"
            >
        ...
        <section>
            ...
            {post.comments.map((c) => (
              <div key={c.id} className="card-pop-sm p-4">
                ...
                {c.media && (
                  <img
                    src={c.media}
                    className="mt-2 max-h-56 rounded-lg border-2 border-[var(--ink)]"
                    alt=""
                  />
                )}
                <div className="mt-2 flex gap-2">
                  <button
                    onClick={() => toggleReact({ postId: post.id, commentId: c.id }, "likes")}
                    className="btn-pop btn-pop-hover bg-card px-2 py-1 text-xs"
                  >
                    👍 {c.likes.length}
                  </button>
                  <button
                    onClick={() => toggleReact({ postId: post.id, commentId: c.id }, "dislikes")}
                    className="btn-pop btn-pop-hover bg-card px-2 py-1 text-xs"
                  >
                    👎 {c.dislikes.length}
                  </button>
          ...
          <div className="card-pop p-4 mt-5">
            ...
            />
            {media && (
              <img src={media} className="max-h-40 rounded-xl border-2 border-[var(--ink)] mt-2" />
            )}
            <div className="mt-2 flex justify-between">
              ...
              <button
                ...
                onClick={() => {
                  addComment(post.id, body.trim(), media);
                  setBody("");
                  setMedia(undefined);
                }}
