    ...
    <div className="grid md:grid-cols-3 gap-6">
      <div className="md:col-span-2">
        ...
        <p className="text-muted-foreground">
          Shared game state, live scoreboard, server-authoritative rounds.
        </p>
        <div className="mt-6 space-y-3">
          {state.lobbies.length === 0 && (
            <div className="card-pop p-6 text-muted-foreground">No open lobbies. Start one →</div>
          )}
          {state.lobbies.map((l) => (
            <div key={l.id} className="card-pop-sm p-4 flex items-center gap-3">
              ...
              <div className="flex-1">
                ...
                <div className="text-xs text-muted-foreground">
                  hosted by @{l.hostName} · {l.players.length} player
                  {l.players.length === 1 ? "" : "s"} · {l.status}
                </div>
              </div>
              <button
                onClick={() => {
                  joinLobby(l.id);
                  navigate(`/lobbies/${l.id}`);
                }}
